...
class _TransactionDetailScreenState extends State<TransactionDetailScreen> {
  ...
  Future<void> _deleteTransaction() async {
    if (_currentTransaction.isGoalMovement ||
        _currentTransaction.isDebtMovement) {
      _showErrorDialog(
        context,
        'Giao dịch này được quản lý trong mục ${_currentTransaction.isGoalMovement ? 'Mục tiêu tiết kiệm' : 'Sổ nợ'}.',
      );
      return;
    }
    final isTransfer = _currentTransaction.isTransfer;
    final confirmed = await showDialog<bool>(
      context: context,
      builder: (ctx) => AlertDialog(
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)),
        title: Text(isTransfer ? 'Xóa chuyển khoản' : 'Xóa giao dịch'),
        content: Text(
          isTransfer
              ? 'Bạn có chắc chắn muốn xóa giao dịch chuyển tiền này không? Số dư của cả 2 ví liên quan sẽ được cập nhật lại.'
              : 'Bạn có chắc chắn muốn xóa giao dịch này không? Số dư ví sẽ được cập nhật lại.',
        ),
        actions: [
          TextButton(
  ...
  void _editTransaction() async {
    if (_currentTransaction.isInternalMovement) return;
    final result = await Navigator.push(
      context,
      MaterialPageRoute(
        builder: (_) =>
            AddTransactionScreen(initialTransaction: _currentTransaction),
      ),
    );
    ...
    if (result == true) {
      ...
      try {
        final updated = updatedTxs.firstWhere(
          (tx) => tx.id == _currentTransaction.id,
        );
        if (mounted) setState(() => _currentTransaction = updated);
      } catch (_) {
        if (mounted) {
          Navigator.pop(context, true); // Tx might have been moved/deleted
        }
      }
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: _isLoading
          ? const Center(
              child: CircularProgressIndicator(color: Color(0xFFE0248A)),
            )
              ...
              child: Column(
                children: [
                  Container(
                    ...
                    child: Column(
                      children: [
                        ...
                        Text(
                          '${isExpense ? '-' : '+'}${NumberFormat('#,###').format(tx.amount)}đ',
                          style: TextStyle(
                            fontSize: 34,
                            fontWeight: FontWeight.bold,
                  ...
                  Padding(
                    ...
                    child: tx.isInternalMovement
                        ? const Text(
                            'Giao dịch nội bộ được quản lý tại nghiệp vụ đã tạo ra nó để bảo toàn số dư và lịch sử.',
  ...
  Widget _buildCategoryIcon(CategoryModel? cat) {
    ...
    return CircleAvatar(
      ...
      child: const Icon(Icons.attach_money, color: Colors.grey, size: 36),
    );
  }
