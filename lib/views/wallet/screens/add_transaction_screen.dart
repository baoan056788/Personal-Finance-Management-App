...
class _AddTransactionScreenState extends State<AddTransactionScreen> {
  ...
  Future<void> _saveTransaction() async {
    ...
    try {
      ...
      if (_isEditing) {
        warningMessage = await _transactionService.updateTransaction(
          _selectedWallet!.id,
          widget.initialTransaction!,
          transaction,
        );
      } else {
        warningMessage = await _transactionService.createTransaction(
          _selectedWallet!.id,
          transaction,
        );
      }
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            _isEditing ? 'Đã cập nhật giao dịch!' : 'Đã thêm giao dịch!',
          ),
                ...
                children: [
                  Row(
                    children: [
                      ...
                      Text(
                        _isEditing ? 'Sửa Giao Dịch' : 'Ghi Chép Giao Dịch',
          ...
          child: Row(
            ...
            children: [
              Icon(
                icon,
                size: 16,
                color: isSelected ? momoPink : Colors.grey.shade500,
              ),
                    ...
                    children: [
                      Container(
                        ...
                        decoration: BoxDecoration(
                          ...
                          border: Border.all(
                            color: isSelected ? momoPink : Colors.transparent,
                            width: 1,
                          ),
                        ),
                        child: Icon(
                          catIcon,
                          color: isSelected ? momoPink : catColor,
                          size: 24,
                        ),
                      ),
                      const SizedBox(height: 8),
